...
# Check Money
    ...
    execute store result score .coinsInventory ka.Values run clear @s sunflower{ka.Coin:1b} 0
    #> If enough money, Buy
    execute store success score .success ka.Values if score .coinsInventory ka.Values >= .resourcesRequired ka.Values
