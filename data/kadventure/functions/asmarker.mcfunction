...
execute if score .mobsEnabled ka.Values matches 1 if score .entityCount ka.Values matches ..20 as @e[type=marker,tag=ka.Center,tag=!ka.NoMobCenter,predicate=kadventure:c30] at @s unless entity @p[distance=..10] run function kadventure:dungeon/sort_mobs